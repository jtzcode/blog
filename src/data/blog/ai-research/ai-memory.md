...
> 现代认知神经科学并不把记忆视为对过去经验的固定读取，而更倾向于将记忆理解为一种**线索驱动的动态重建**过程。过去经历会留下分布式的记忆痕迹（Engram）；当情境、内部状态、语义关联或目标与这些痕迹形成足够匹配时，相关神经活动模式被重新激活，并与既有知识和图式（Schema）重新组合，**生成**当下这一次记忆。因此，记忆既不是纯粹复现，也不是任意虚构，而是**在历史痕迹约束下的重构**。[^1] [^2] [^3] [^4]
...
在这一点上，人类的**海马体**和**新皮层**系统就很高级。新皮层可以在睡眠时对海马体中的新记忆进行反复重放和巩固，优化新皮层中的神经连接，把新记忆转为长期记忆[^5] [^7]。这相当于对**神经网络的权重参数**进行调整，以体现新的记忆。

第三种就是我们常见的、基于存储和检索的记忆方案。这是静态的思路。

对于第二种，一个典型案例是 EverMind 公司提出的 **MSA** （Memory Sparse Attention）架构[^6]。
...
- 显式程序记忆 = Agent 的源代码（Prompt 模板、工具调用代码、决策流程）
...
学界和业界与 Agent 记忆相关的研究和实践非常丰富，本文只是从部分角度切入，进行学习和思考，并且结合人类记忆机制进行对比分析。希望能给大家提供一些启发和参考。欢迎讨论交流。👏
